    ...
    self.request({
      ...
      if (data.screenshots[0].state === 'processing'){
        ...
        if (data.screenshots[index].state === 'done'){
          ...
          console.log('Image %d still waiting', index +1);
          self.queryQueue.apply(self, [data.wait_time,jobID, index,cb]);
        }
      }
    });
...
APIClient.prototype.fetchImages = function(data,cb){
  ...
  downloader.getImage(data.image_url, self.getImageName(data), cb);
};
/**
      ...
      if (browserStackBrowser.os.length === 1 || !os || arrayInArray(os,browserStackBrowser.os)){
        for (var i = 0, len = browserStackBrowser.list.length; i < len; i++){
          var currentBrowserVersion = parseFloat(browserStackBrowser.list[i].browser_version).toFixed(1) || null,
              ...
              currentBrowserOSVersion = browserStackBrowser.list[i].os_version,
          ...
          if (self.validBrowserVersion(currentBrowserVersion,browserVersion)){
            if (self.validOS(browserStackBrowser, currentBrowserOSName, os)){
              if (self.validOSVersion(osVersions, currentBrowserOSName, currentBrowserOSVersion)){
 ...
 * @returns {boolean}
 */
APIClient.prototype.validOSVersion = function(osVersions, currentBrowserOSName, currentBrowserOSVersion){
  return !osVersions || (typeof osVersions[currentBrowserOSName] !== "undefined" && (osVersions[currentBrowserOSName].indexOf(currentBrowserOSVersion) > -1 || !osVersions[currentBrowserOSName].length));
};
/**
